...
// Representación de una tripleta (w, x, y) perteneciente a M 
struct Tripleta {
    std::string w;
    std::string x;
    std::string y;
    std::string tipo; // Para identificar si es Verdad, Falso, Clausula o Basura
};

// Estructura para el problema 3SAT
struct Clausula {
    int l1, l2, l3; // Literales: p=1, -p=-1, q=2, -q=-2, etc.
};
...
private:
    ...
    std::vector<Tripleta> M; // El conjunto M final [cite: 7]
...
public:
    Reduccion3SATto3DM(int numVars, std::vector<Clausula> f) : n(numVars), formula(f) {
        m = formula.size();
    }
...
int main() {
    // Definición del problema ejemplo de los apuntes:
    // (p v -q v -r) ^ (-p v -q v r) [cite: 22, 41]
    
    // Mapeo: p=1, q=2, r=3. Negativos con signo -.
    std::vector<Clausula> ejemplo = {
        {1, -2, -3}, // (p v -q v -r)
        {-1, -2, 3}  // (-p v -q v r)
    };

    int numVariables = 3; // p, q, r

    Reduccion3SATto3DM reduccion(numVariables, ejemplo);
    reduccion.generar();
